...
from Begin_Func import DenoisingDataset, EarlyStopping, process_image_2, operateur, load_img, PSNR
...
def main():
    ...
    transforms.Resize((256, 256)),               # Redimensionner à (40, 40)
    ...
    valid_loader = DataLoader(dataset=valid, batch_size=256, shuffle=False)
    ...
    #         # Forward pass
    #         outputs = denoise(noisy_images, model, t_steps=50, train=True)
    ...
    #     lmbd = CRRNN().lmbd_transformed
    #     print(f"Lambda: {lmbd:.2f}")
    #     mu = CRRNN().mu_transformed
    #     print(f"Mu: {mu:.2f}")
        # i=0
        ...
        # denoised_image = denoise(noisy_images[i], CRRNN(), t_steps=100, operator_type ="none", operator_params=None, train=True, auto_params=False, lmbd=10.5e-0, mu=3e-0, step_size=1e-1).squeeze(0).permute(1, 2, 0).cpu().numpy()
    ...
    with torch.no_grad():
        ...
        debblurred_image = process_image_2(Im_butterfly_blurred_high, operator=denoise, model=CRRNN(), t_steps=200, operator_type="convolution", operator_params={"G": G_butterfly_high}, auto_params=False, lmbd=2e-1, mu=5e-0, step_size=2)
    ...
    psnr_denoised = PSNR(debblurred_image, truth_image, max_intensity=1.0)
    ...
    plt.imshow(debblurred_image, cmap="gray")
    plt.subplot(1, 3, 3)
