...
from Begin_Func import DenoisingDataset, EarlyStopping
...
def main():
    ...
    # transforms.Resize((256, 256)),               # Redimensionner à (256, 256)
    ...
    train_dataset = DenoisingDataset(images_dir=images_dir, image_files=image_files, transform=transform, noise_level=0.2, random=False)
    ...
    num_train_0 = int(len(train_0) * 0.8)

    train, valid = \
        random_split(train_0, [num_train_0, len(train_0) - num_train_0])

    # DataLoader (Batch size = 1) # Batch size = 16
    train_loader = DataLoader(dataset=train, batch_size=16, shuffle=True)
    valid_loader = DataLoader(dataset=valid, batch_size=16, shuffle=False)
    ...
    for i in range(5):
        ...
        axes[1, i].imshow(images_truth[i].permute(1, 2, 0), cmap="gray")  # Permuter les dimensions pour imshow
